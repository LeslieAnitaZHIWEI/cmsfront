<template>
 ...
 <Form :rules="loginRules" v-show="!toggle"  ref="formLogin" style="margin-top:25px" :model="formLogin" :label-width="40">
        ...
        <FormItem label=""  prop="reg">
           <a @click="toggle=true">注册账号</a>
        </FormItem>
    </Form>
             ...
             <Form :rules="ruleCustom" v-show="toggle"  ref="formItem" :model="formItem" class="loginForm" :label-width="80">
        ...
        <FormItem label="确认密码" prop="passwdCheck">
            <Input type="password" v-model="formItem.passwdCheck"  placeholder="请确认密码"  />
        </FormItem>
        <FormItem label="验证码"  prop="code">
            <Input v-model="formItem.code" placeholder="请输入验证码" style="width: 120px" /> <Button @click="getCode">发送验证码</Button>
        </FormItem>
         <FormItem>
            <Button size="large" style="width:200px" type="primary" @click="handleSubmit('formItem')">注册</Button>
        </FormItem>
    </Form>
    ...
    getCode() {
      this.$refs["formItem"].validateField("email", valid => {
        ...
        if (valid) {
          ...
          this.$http({
            url: "/admin/email/" + this.formItem.email,
            method: "post"
            // params: {code:this.formItem.code},
            // data:{
            //     ...this.formItem
